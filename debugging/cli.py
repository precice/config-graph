...
def main():
    ...
    parser = argparse.ArgumentParser(
        usage="%(prog)s",
        description="Creates a graph from a config.xml file for preCICE.",
    )
    parser.add_argument(
        "src", type=argparse.FileType("r"), help="Path of the config.xml source file."
    )
    ...
    if args.src.name.endswith(".xml"):
        ...
        sys.exit(
            f"[{color_red}ERROR{color_reset}]: '{color_cyan}{args.src.name}{color_reset}' is not an xml file"
        )
