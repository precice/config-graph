...
def test_graph():
    xml = xml_processing.parse_file(
        "tests/example-configs/simple-good/precice-config.xml"
    )
    ...
    n_participant_generator = n.ParticipantNode(
        "Generator", None, None, None, [n_mesh_generator], None, None, None, None, None
    )
    n_participant_propagator = n.ParticipantNode(
        "Propagator",
        None,
        None,
        None,
    ...
    # Write Data
    n_write_data_generator_color_generator_mesh = n.WriteDataNode(
        n_participant_generator, n_data_color, n_mesh_generator
    )
    n_participant_generator.write_data = [n_write_data_generator_color_generator_mesh]

    # Read Data
    n_read_data_propagator_color_propagator_mesh = n.ReadDataNode(
        n_participant_propagator, n_data_color, n_mesh_propagator
    )
    n_participant_propagator.read_data = [n_read_data_propagator_color_propagator_mesh]

    # Receive Mesh
    n_receive_mesh_propagator_generator_mesh_generator = n.ReceiveMeshNode(
        n_participant_propagator, n_mesh_generator, n_participant_generator, False
    )
    n_participant_propagator.receive_meshes = [
        n_receive_mesh_propagator_generator_mesh_generator
    ]

    # Mappings
    n_mapping_propagator = n.MappingNode(
    ...
    # Coupling Schemes
    n_coupling_scheme_generator_propagator = n.CouplingSchemeNode(
    ...
    # m2n
    n_m2n_generator_propagator = n.M2NNode(
        M2NType.SOCKETS,
        n_participant_generator,
        n_participant_propagator,
    )
    ...
    edges += [
        (
            n_participant_generator,
            n_mesh_generator,
            Edge.PROVIDE_MESH__PARTICIPANT_PROVIDES,
        )
    ]
    edges += [
        (
    ...
    edges += [
        (
            n_m2n_generator_propagator,
            n_participant_generator,
            Edge.M2N__PARTICIPANT_ACCEPTOR,
        )
    ]
    edges += [
        (
            n_m2n_generator_propagator,
            n_participant_propagator,
            Edge.M2N__PARTICIPANT_CONNECTOR,
        )
    ]
    ...
    edges += [
        (
            n_coupling_scheme_generator_propagator,
            n_participant_generator,
            Edge.COUPLING_SCHEME__PARTICIPANT_FIRST,
        )
    ]
    ...
    edges += [
        (
            n_coupling_scheme_generator_propagator,
            n_participant_propagator,
            Edge.COUPLING_SCHEME__PARTICIPANT_SECOND,
        )
    ]
    ...
    edges += [
        (
            n_write_data_generator_color_generator_mesh,
            n_data_color,
            Edge.WRITE_DATA__WRITES_TO_DATA,
        )
    ]
    ...
    edges += [
        (
            n_write_data_generator_color_generator_mesh,
            n_mesh_generator,
            Edge.WRITE_DATA__WRITES_TO_MESH,
        )
    ]
    ...
    edges += [
        (
            n_data_color,
            n_read_data_propagator_color_propagator_mesh,
            Edge.READ_DATA__DATA_READ_BY,
        )
    ]
