...
def test_graph():
    ...
    n_generator_write_data = n.WriteDataNode(
        n_generator_participant, n_color, n_generator_mesh
    )
    ...
    edges += [
        ...
        (
            n_generator_mesh,
            n_generator_participant,
            Edge.PROVIDE_MESH__PARTICIPANT_PROVIDES,
        ),
    ]
    edges += [
        (
            n_generator_write_data,
            n_generator_participant,
    ...
    n_mapping = n.MappingNode(
        ...
        from_mesh=n_generator_mesh,
        to_mesh=n_propagator_mesh,
    )
    n_propagator_participant.mappings.append(n_mapping)

    n_read_data = n.ReadDataNode(
        n_propagator_participant, data=n_color, mesh=n_propagator_mesh
    )
    n_propagator_participant.read_data.append(n_read_data)

    n_action = n.ActionNode(
        n_propagator_participant,
        type=ActionType.MULTIPLY_BY_AREA,
        mesh=n_propagator_mesh,
        timing=TimingType.WRITE_MAPPING_POST,
        target_data=n_color,
        source_data=[],
    )
